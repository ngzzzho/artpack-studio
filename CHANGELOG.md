...
## v0.11.0 — 2026-08-20
- 六屬性 + GK icon 升級 CR 式多色小插畫 prompt（主色對應屬性色），
  解除「現貨已覆蓋」跳過 — 卡面套裝 ⚡ 一鍵而家會出：金屬箔×4 + 彩色icon×12
- 預計一鍵費用 ~US$0.65（13 張 AI 圖，fal 標準）
...
- 卡面套裝加「紫金卡底放射紋」（Card_BG_Rays，3:4 全幅）— UI 已埋好 hook，
  生成+出品後紫金卡自動着燈
- 提示：「場景」類 BG_Pitch/BG_Locker/BG_Podium 係下一步質感關鍵（詳情頁背景）
...
- **⚡ 一鍵生成掣**（每個類別標題度都有）：一撳出晒成類未做嘅項目，
  先報「幾多項、AI 幾多張、大約幾錢」你 confirm 先開波；進度喺「啱啱生成」面板睇
- 已完成項目自動跳過（想重出逐項撳「再生成」，唔會誤燒錢）

## v0.9.3 — 2026-08-20
- **全局零文字鎖**：所有生成 prompt 自動附加「絕對唔准有文字/字母/數字」
  — 文字一律喺 Godot 層加，自由縮放唔跟圖像比例（Emma 鐵則）

...
// 鐵則：圖像零文字 — 文字一律喺 Godot 層加（自由縮放、換字唔跟圖變形）
const NO_TEXT = '. ABSOLUTELY NO text, letters, numbers, words or typography anywhere in the image — leave labelled areas completely blank';
...
export async function startSetJob({ item, provider, tier = 'std', customPrompt, userRefs }) {
  ...
  const outCat = item.outCat || item.cat;
  const outDirRel = item.kind === 'single'
    ? path.join(OUTPUT_PACK, outCat)
    : path.join(OUTPUT_PACK, outCat, item.folder || item.file);
  const job = {
...
async function runJob(job, item, cfg) {
  ...
  let masterNine = null; // master 嘅九宮格分析結果 — 變體直接繼承 margins
  ...
  for (const step of job.steps) {
    ...
    try {
      ...
      if (step.local === 'metal') {
        ...
        step.images.push(path.join(job.outDir, step.file));
        if (masterNine) {
          const vbase = step.file.replace(/\.png$/, '');
          fs.writeFileSync(path.join(outAbs, `${vbase}_slices.json`), JSON.stringify(masterNine, null, 2));
        }
        step.status = 'done';
        continue;
      }
